...
static void on_write(ble_cus_t * p_cus, ble_evt_t const * p_ble_evt)
{
  ...
  /* Low Power */
    // Low Power Value Characteristic Written to.
    if (p_evt_write->handle == p_cus->low_power_value_handles.value_handle)
    {
        __ALIGN(4) uint8_t temp_array[2] = {p_evt_write->data[0]};
        ble_cus_low_power_value_update(p_cus, temp_array, 1);
...
static uint32_t ble_char_add(ble_cus_t * p_cus, const ble_cus_init_t * p_cus_init, uint16_t UUID)
{
    uint32_t            err_code;
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_md_t cccd_md;
    ...
    err_code = sd_ble_gatts_characteristic_add(p_cus->service_handle, &char_md,
                                               &attr_char_value,
                                               &p_cus->low_power_value_handles.value_handle);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
uint32_t ble_cus_init(ble_cus_t * p_cus, const ble_cus_init_t * p_cus_init)
{
    ...
    err_code = ble_char_add(p_cus, p_cus_init, LOW_POWER_UUID);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    } 
...
uint32_t ble_cus_custom_value_update(ble_cus_t * p_cus, uint8_t * custom_value)
{
    ...
    __ALIGN(4) uint8_t temp_array[4] = {custom_value[0], custom_value[1], 0x00, 0x00};
...
uint32_t ble_cus_low_power_value_update(ble_cus_t * p_cus, uint8_t * current_value, uint8_t len)
{
    ...
    err_code = kls_fds_find_and_delete(LOWP_FILE_ID_FDS, LOWP_REC_KEY_FDS);
    APP_ERROR_CHECK(err_code);

    err_code = kls_fds_write(LOWP_FILE_ID_FDS, LOWP_REC_KEY_FDS, current_value);
    APP_ERROR_CHECK(err_code);

    NRF_LOG_INFO("Wrote: %x", current_value[0]);
    //p_cus->current_value = current_value[0];
    ...
    if ((p_cus->conn_handle != BLE_CONN_HANDLE_INVALID)) 
    {
        ...
        hvx_params.handle = p_cus->low_power_value_handles.value_handle;
        hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
        hvx_params.offset = gatts_value.offset;
        hvx_params.p_len  = &gatts_value.len;
        hvx_params.p_data = gatts_value.p_value;
    ...
    p_cus->low_power_value = *current_value;
